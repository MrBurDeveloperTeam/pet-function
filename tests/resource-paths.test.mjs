...
test('Built-in beds ignore stale catalog URLs; custom beds retain their images', () => {
  for (const color of ['grey', 'red', 'purple']) assert.equal(images.resolveBedImage('bed_' + color, '/images/broken.png'), '/pet-function/pet/' + color + '_bed.png');
...
test('Bed fallback is finite, deduplicated, and ends with network-independent artwork', () => {
  ...
  assert.equal(candidates[1], '/pet-function/pet/grey_bed.png');
  assert.ok(candidates.at(-1).startsWith('data:image/svg+xml,'));
  assert.equal(images.bedImageCandidates('/pet-function/pet/grey_bed.png').length, 2);
});

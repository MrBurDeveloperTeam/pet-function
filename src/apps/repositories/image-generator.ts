// Preserved image-generator database mapping. The host injects its authenticated client.
...
export function createContentStudioPetRepository(supabase: PetDatabaseClient): PetRepository {
  return {
  async loadSnapshot(userId: string): Promise<PetSaveSnapshot | null> {
    ...
    if (error) {
      console.error('[contentStudioPetRepository] Failed to load inventory_pet:', error);
      return null;
    }
    if (!data) return null;
  ...
  async loadInventoryRows(userId: string): Promise<PetInventoryItem[]> {
    ...
    if (error) {
      console.error('[contentStudioPetRepository] Failed to load pet_inventory:', error);
      return [];
    }
  ...
  async loadCatalog(): Promise<FoodItem[]> {
    ...
    if (error || !data || data.length === 0) {
      if (error) console.error('[contentStudioPetRepository] Failed to load aiboard_pricing_items:', error);
      return [];
    }

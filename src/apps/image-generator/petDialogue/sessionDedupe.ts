...
//
// CROSS-TAB "dismissed" state — localStorage, shared across all Content
// Studio tabs on this origin. Written ONLY on an explicit user action:
// clicking Close or the reminder's CTA. Purpose: once the user has
// actually acted on the reminder in any one tab, no other already-open
// tab or later-opened tab — nor that SAME tab after a full reload — may
// show that same instance again.
//
...
// (`isDialogueSeenThisSession`/`markDialogueSeenThisSession`, composed via
// `isDialogueIneligible`) — that was removed because sessionStorage
// SURVIVES a same-tab full page reload (standard, spec'd browser
// behavior), so it was incorrectly suppressing a candidate the user had
// never actually Closed/CTA'd, on every subsequent reload. Persisted
// storage is the wrong layer for "don't reopen within this one mount";
// only genuinely explicit handling belongs in storage that outlives the
// mount.
//
// Namespaced by BOTH the authenticated Supabase user id AND the
// candidate's own `dedupeKey`, so a different candidate instance (e.g. a
// different generation id) is never suppressed by an unrelated dismissal,
...
//
// SAME-APP ONLY: localStorage is origin-scoped. A dismissal here can never
// suppress the same reminder on a different Snabbb origin (e.g.
 ...
 *  action — never at show-time. See CatMascot.jsx's `shownCandidateKeysRef`
 *  for the separate, non-persisted, mount-scoped show-time mark. */

/**
 * Ported verbatim from Content Studio's `src/VirtualPet/constants.ts`.
 * Pure data — no Supabase/host coupling. `FOOD_ITEMS` here is the static
 * fallback catalog (identical to Content Studio's own fallback before its
 * `aiboard_pricing_items` fetch resolves) — the live catalog is supplied at
 * runtime by the host's `PetRepository.loadCatalog()`.
 */
import bedGreyUrl from '../../assets/pet/grey_bed.png';
import bedRedUrl from '../../assets/pet/red_bed.png';
import bedPurpleUrl from '../../assets/pet/purple_bed.png';
import type { PetStats, RoomType as RoomTypeT, FoodItem, ToyItem, BedItem } from './types';
...
export const ROOM_THEMES: Record<RoomTypeT, { bg: string; accent: string; icon: string }> = {
...
export const BED_ITEMS: BedItem[] = [
  { id: 'bed_grey', label: 'Grey Bed', price: 0, src: bedGreyUrl, energyGain: 2, levelReq: 1 },
  { id: 'bed_red', label: 'Red Bed', price: 300, src: bedRedUrl, energyGain: 3, levelReq: 1 },
  { id: 'bed_purple', label: 'Purple Bed', price: 600, src: bedPurpleUrl, energyGain: 4, levelReq: 1 },
];
